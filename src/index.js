var isArray = require("is_array"),
    trim = require("trim"),
    base64 = require("base64_encoding");
...
Buffer.prototype.copy = function(target, target_start, start, end) {
    ...
    if (end < start) {
        throw new Error("Buffer.copy(target, target_start, start, end) sourceEnd < sourceStart");
    }
    if (target_start >= 0 && target_start >= target.length) {
        throw new Error("Buffer.copy(target, target_start, start, end)targetStart out of bounds");
    }
    if (start >= 0 && start >= this.length) {
        throw new Error("Buffer.copy(target, target_start, start, end)sourceStart out of bounds");
    }
    if (end >= 0 && end > this.length) {
        throw new Error("Buffer.copy(target, target_start, start, end)sourceEnd out of bounds");
    }
...
Buffer.prototype.fill = function(value, start, end) {
    ...
    if (end < start) {
        throw new Error("Buffer.fill(value, start, end) end < start");
    }
    if (this.length === 0 || end === start) {
        return this;
    }

    if (start >= 0 && start >= this.length) {
        throw new Error("Buffer.fill(value, start, end) start out of bounds");
    }
    if (end >= 0 && end > this.length) {
        throw new Error("Buffer.fill(value, start, end) endout of bounds");
    }
...
Buffer.prototype.equals = function(b) {
    if (!Buffer.isBuffer(b)) {
        throw new Error("Buffer.equals(b) Argument must be a Buffer");
    }
...
Buffer.prototype.hexWrite = function(string, offset, length) {
    ...
    if (strLen % 2 !== 0) {
        throw new Error("Buffer.hexWrite(string, offset, length) Invalid hex string");
    }
...
Buffer.prototype.base64Write = function(string, offset, length) {
    return blitBuffer(this, base64.stringToBytes(string), offset, length);
};
...
Buffer.byteLength = function(str, encoding) {
    ...
    if (encoding === "utf8" || encoding === "utf-8") {
        ...
        return base64.stringToBytes(str).length;
    } else {
...
Buffer.compare = function(a, b) {
    ...
    if (!Buffer.isBuffer(a) || !Buffer.isBuffer(b)) {
        throw new Error("Buffer.compare(a, b) Arguments must be Buffers");
    }
    x = a.length;
    y = b.length;
    i = 0;
...
function blitBuffer(out, src, offset, length) {
    ...
    while (i < length) {
        if ((i + offset >= outLength) || (i >= srcLength)) {
            break;
        }
        out[i + offset] = src[i];
        i++;
